...
if( function_exists( 'bogo_languages' ) ) {
  ...
  function custom_bogo_language_switcher( $args = '' ) {
  	$args = wp_parse_args( $args, array(
  		'echo' => false,
  	) );

  	$links = custom_bogo_language_switcher_links( $args );
  	$total = count( $links );
  	$count = 0;
  	...
  	foreach ( $links as $link ) {
        ...
        // $li = bogo_lang_slug( $link['locale'] );
  		...
  		if ( apply_filters( 'bogo_use_flags', true ) ) {
  			...
  			// $flag = sprintf(
  			// 	'<span class="c-bogoflags bogoflags-%s"></span>',
  			// 	$flag ? $flag : 'zz' );
        //
  	...
  	$output = '<ul class="custom-bogo-language-switcher">' . $output . '</ul>' . "\n";
  ...
  function custom_bogo_language_switcher_links( $args = '' ) {
  	global $wp_query;
  	...
  	return apply_filters( 'custom_bogo_language_switcher_links', $links, $args );
  }
}
